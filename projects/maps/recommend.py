...
def group_by_centroid(restaurants, centroids):
    ...
    for i in restaurants:
        closest += [[find_closest(restaurant_location(i), centroids), i]]
...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        centroid_lat += [restaurant_location(restaurant)[0]]
        centroid_lon += [restaurant_location(restaurant)[1]]
    return [mean(centroid_lat), mean(centroid_lon)]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    while prev_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        cluster_group = group_by_centroid(restaurants, prev_centroids)
        centroids = [find_centroid(i) for i in cluster_group]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    predictor_lst = [find_predictor(user, reviewed, i) for i in feature_fns]
    best_pred, r_squared = max(predictor_lst, key = lambda x: x[1])
    return best_pred
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for i in restaurants:
        if i in reviewed:
            rate_dictionary[restaurant_name(i)] = user_rating(user, restaurant_name(i))
        else:
            rate_dictionary[restaurant_name(i)] = predictor(i)
