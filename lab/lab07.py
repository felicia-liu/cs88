...
class T88ble():
    ...
    def with_column(self, label, values):
        ...
        i = 0
        updated_rows = []
        updated_labels = self.labels() + [label]
        for j in self.rows:
            updated_rows += [j + [values[i]]]
            i += 1
        return T88ble(updated_rows, updated_labels)
    ...
    def select(self, labels):
        ...
        indices = []
        selected = []
        copy_rows = self.rows.copy()
        for i in labels:
            index = self.column_labels.index(i)
            indices += [index]
        for j in copy_rows:
            selected += [[ j[k] for k in indices]]
        return T88ble(selected, labels)
    ...
    def sort(self, label, descending=True):
        ...
        sorted_rows = []
        x = sorted(self.rows, key=lambda row: row[index])
        if descending==True:
            sorted_rows = x[::-1]
        else:
            sorted_rows = x
